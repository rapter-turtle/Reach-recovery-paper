...
The reach avoid value and the two actors were trained on the shared model with discount $\gamma=0.999$, an $N=5$ step target, and a control period of 0.2 s, under the disturbance box $|w_u|\le 0.13$, $|w_v|\le 0.07$, and $|w_r|\le 0.04$ played by the adversary. The learned policy is compared against the Phase 1 DOB-MPC with RaCBF carried through the docking \citep{park2026recovery}, evaluated at a tuned and a conservative barrier gain, together with an ALOS CBF guidance baseline and a nominal reach avoid policy trained without the adversary. Every controller ran the same 100 sea scenarios, each a physical wind, wave, and current field drawn from a Fossen sea model inside the training box, with identical initial states and disturbance seeds, so the controllers differ only in their law. The reported metrics are the number of scenarios docked, the collisions, the timeouts, the median recovery time, and the worst case clearance to the obstacles.

The proposed two phase recovery docked all 100 scenarios with 0 collisions and 0 timeouts, a median recovery time of 33 s, and a worst case clearance of 0.69 m, so it stayed well clear of the obstacles. The DOB-MPC with RaCBF was fragile, and its outcome swung with the barrier gain. At the tuned gain it docked 95 of 100 but struck the structure in 5, with a worst clearance of $-0.003$ m, since near the pocket the barrier demanded lateral corrections that the unactuated sway could not deliver. At the conservative gain it had 0 collisions but held the hull off the cradle and timed out in 2 of 100, with a worst clearance of 0.41 m. A gain sweep showed the dependence is not monotone, moving from 7 collisions at the weakest gain, through a narrow band that docked all 100, to 2 timeouts at the strongest, so only a delicately hand tuned gain reached full success, and even then with far less clearance than the proposed method. The ALOS CBF guidance likewise depends on its gains. Because the two phase docking carries the reach avoid value rather than a barrier gain, its result held at every gain, and the setting that cost the baseline five hulls was invisible to it. Representative closed loop trajectories are shown in Fig.~\ref{fig_trajectories} and the aggregate statistics are summarized in Table~\ref{tab_montecarlo}.
...
\begin{table}[t]
  \centering
  \caption{Monte Carlo comparison over 100 sea scenarios. Dock, collision, and timeout counts, the median recovery time, and the worst case clearance to the obstacles.}
  \label{tab_montecarlo}
  \resizebox{\linewidth}{!}{%
  \begin{tabular}{lccccc}
    \hline
    Controller & Dock & Collision & Timeout & Median time [s] & Worst clearance [m] \\
    \hline
    Proposed (two phase) & 100 & 0 & 0 & 33.0 & $+0.69$ \\
    DOB-MPC + RaCBF, tuned & 95 & 5 & 0 & 39.7 & $-0.003$ \\
    DOB-MPC + RaCBF, conservative & 98 & 0 & 2 & 41.6 & $+0.41$ \\
    ALOS CBF & & & & & \\
    Nominal reach avoid & & & & & \\
    \hline
  \end{tabular}}
