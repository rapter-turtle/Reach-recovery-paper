...
% body origin and heading reference
\coordinate (Ob) at (3.5,2.4);
\draw[dashed] (Ob) -- ++(0,1.5);
\draw[->] ($(Ob)+(0,1.3)$) arc (90:55:1.3) node[midway,above]{$\psi$};
\begin{scope}[shift={(Ob)}, rotate=55]
  % RHIB hull with a pointed bow at +x
  \draw[thick, fill=blue!6]
    (-1.0,0.45) -- (0.85,0.45) .. controls (1.25,0.30) and (1.5,0.12) .. (1.65,0)
    .. controls (1.5,-0.12) and (1.25,-0.30) .. (0.85,-0.45) -- (-1.0,-0.45)
    .. controls (-1.2,-0.25) and (-1.2,0.25) .. cycle;
  % inner tube outline of the RHIB
  \draw[blue!35, thick] (-0.8,0.30) -- (0.8,0.30) .. controls (1.05,0.18) .. (1.25,0)
    .. controls (1.05,-0.18) .. (0.8,-0.30) -- (-0.8,-0.30);
  % body origin
  \fill (0,0) circle (1.4pt);
  \node[above left=-2pt] at (0,0) {$O_b$};
  % body axes
  \draw[->] (0,0) -- (2.2,0) node[right]{$u$};
  \draw[->] (0,0) -- (0,-1.25) node[below]{$v$};
  % yaw rate
  \draw[->] (0.55,0.55) arc (45:-35:0.78) node[midway,right]{$r$};
  % moment arm to the thruster
  \draw[green!50!black, dashed] (0,0) -- (-1.0,0);
  \node[green!45!black, above] at (-0.5,0) {$l_r$};
  % head point
  \fill[red!70!black] (1.95,0) circle (1.2pt);
  \node[red!60!black, above] at (1.95,0) {$l_h$};
  % stern thruster with deflection delta and thrust F
  \draw[dashed] (-1.0,0) -- (-1.9,0);
  \draw[->, thick, red!70!black] (-1.0,0) -- (-1.72,-0.42) node[below left]{$F$};
  \draw[->] (-1.5,0) arc (180:210:0.5) node[left]{$\delta$};
\end{scope}
\end{tikzpicture}
\caption{Coordinate frames and geometry of the USV. The inertial frame $O_i$ and the body frame $O_b$ carry the surge $u$, sway $v$, and yaw rate $r$, the heading $\psi$, the stern thrust $F$ at deflection $\delta$, the moment arm $l_r$ to the thruster, and the head point at a distance $l_h$ forward of the center of gravity.}
...
\end{equation}
A state dependent rate clamp shrinks the admissible rate near the amplitude bounds so that the actuator states never leave $\mathcal{U}$. The identified parameter values and the numeric bounds are listed in Table~\ref{tab_params} in Section~\ref{sec_results}.
