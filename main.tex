...
The recovery runs in two phases, shown in Fig.~\ref{fig_scenario}. Phase 1 covers the approach from far away and the dynamic positioning alongside the mothership. Because the environmental load varies slowly there and the geometry is open, Phase 1 uses a disturbance observer based model predictive controller with a robust adaptive control barrier function \citep{park2026recovery}, in which the disturbance observer estimates the load, the model predictive controller carries the estimate inside its prediction model to reject it while respecting the actuator amplitude and rate limits over a horizon, and the robust adaptive control barrier function keeps the USV inside an admissible corridor toward the dynamic positioning point. Phase 2 is the terminal docking into the narrow cradle. Because underactuation and actuator saturation there make it hard to guarantee that an admissible input still exists, Phase 2 is carried out by the robust reach avoid policy of Section~\ref{sec_rl}.

The two phases are connected by a supervisor, whose state machine is shown in Fig.~\ref{fig_flowchart}. The Phase 2 policy of Section~\ref{sec_rl} is obtained together with the value function $V$ of the Hamilton Jacobi reach avoid problem. The reach avoid set is the negative sublevel set of this value, so $V(s)<0$ marks exactly the states that belong to it, the states from which the USV can be brought into the target while avoiding the failure set under the worst case disturbance, and $V(s)\ge 0$ marks the states outside the set. The supervisor watches this sign. Phase 1 hands control to the Phase 2 policy the instant $V(s)<0$, and control falls back to Phase 1 when $V(s)\ge 0$ before the cradle is entered, so a single scalar test governs both the transition and the abort. A disturbance larger than predicted can arrive suddenly, or a communication or mechanical fault can occur, and either can drive the state out of the reach avoid set during docking, so the sign test returns control to Phase 1 and the approach is retried.
