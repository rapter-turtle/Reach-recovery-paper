...
The reach avoid value and both adversarial actors were trained on the shared vehicle model with discount $\gamma=0.999$, an $N=5$ step target, and a control period of 0.2 s, under the adversary disturbance box $|w_u|\le 0.13$, $|w_v|\le 0.07$, and $|w_r|\le 0.04$. The learned policy is compared against the Phase 1 DOB-MPC with RaCBF carried through the docking \citep{park2026recovery}, evaluated at two barrier gains, a nominal gain (1) with poles 0.04 and 0.40 and a stronger gain (2) with poles 0.10 and 0.70, together with an ALOS CBF guidance baseline. Every controller ran the same set of physical sea scenarios, each a wind, wave, and current field drawn from a Fossen sea model, with identical initial states and disturbance seeds, so the controllers differ only in their control law. The docked, collision, and timeout counts, the recovery time, and the worst case clearance to the obstacles are summarized in Fig.~\ref{fig_montecarlo}, and the per call computation time in Table~\ref{tab_compute}.

The proposed two phase recovery docked every scenario with no collision and no timeout and stayed clear of the obstacles throughout. Its Phase 2 reach avoid set control was also the fastest and most consistent to compute, since each action is a single network evaluation with no online optimization. The DOB-MPC with RaCBF depended strongly on the barrier gain. At the nominal gain it docked nearly all scenarios but struck the structure in a few, because near the pocket the barrier demands lateral corrections that the unactuated sway cannot deliver, while at the stronger gain the collisions vanished but the barrier held the hull off the cradle and a few scenarios timed out instead. A sweep of the gain showed that this dependence is not monotone, passing from collisions at the weakest settings through a narrow band that docked every scenario to timeouts at the strongest, so only a delicately tuned gain reached full success. Because the two phase policy carries the reach avoid value rather than a barrier gain, its outcome was invariant to this tuning. The ALOS CBF guidance was weakest, failing to dock untuned and recovering only a fraction of the scenarios even after heavy retuning, with many collisions and timeouts. A representative recovery is shown in Fig.~\ref{fig_representative}, where the thrust and rudder stay within their limits and the clearance remains positive throughout, the trajectories of the controllers over common scenarios appear in Fig.~\ref{fig_trajectories}, the outcome, the recovery time, and the worst clearance are summarized in Fig.~\ref{fig_montecarlo}, and the per call computation time is reported in Table~\ref{tab_compute}.
