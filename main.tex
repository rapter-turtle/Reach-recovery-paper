...
The recovery runs in two phases, shown in Fig.~\ref{fig_scenario}. Phase 1 covers the approach from far away and the dynamic positioning alongside the mothership, and is handled by a disturbance observer based model predictive controller with a reach avoid control barrier function \citep{park2026recovery}. This controller fits Phase 1 because the environmental load varies slowly during the far approach, so a disturbance observer estimates it and the model predictive controller carries the estimate inside its prediction model to reject it while respecting the actuator amplitude and rate limits over a horizon, while the reach avoid control barrier function keeps the USV inside an admissible corridor toward the dynamic positioning point. The combination is well suited to the open water approach and to holding station near the mothership. Phase 2 is the terminal docking into the narrow cradle, where underactuation and actuator saturation make it hard to guarantee that an admissible input still exists, so it is carried out by the robust reach avoid policy of Section~\ref{sec_rl}.

The two phases are connected by a supervisor, whose state machine is shown in Fig.~\ref{fig_flowchart}. The policy of Section~\ref{sec_rl} comes with a learned value function $V$ that assigns a single number to every state. Its sign carries the guarantee. $V(s)$ is negative exactly on the states from which the trained policy can still bring the head point into the target while keeping both the center of gravity and the head point clear of the failure set under the worst case disturbance, so $V(s)<0$ certifies that recovery from $s$ will succeed even against the worst disturbance, whereas $V(s)\ge 0$ means recovery is not certified. The supervisor watches this sign. Phase 1 hands control to the Phase 2 policy the instant $V(s)<0$, and if the disturbance or an imperfect handoff drives the state back to $V(s)\ge 0$ before the cradle is entered, control falls back to the Phase 1 controller and the approach is retried, so a single scalar test governs both the transition and the abort. Because $V$ is computed over the state augmented with the actuator variables $\delta$ and $F$, the test accounts for the thrust and rudder actually available at the switching instant, which matters because a full thrust slew is slow relative to the docking.
