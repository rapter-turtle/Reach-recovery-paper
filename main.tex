...
The reach avoid recovery problem posed earlier is naturally solved by Hamilton Jacobi reachability analysis \citep{mitchell2005time, bansal2017hamilton, lygeros2004reachability}. The USV is underactuated with a single stern thruster and a rudder, it faces a bounded environmental disturbance, and it must reach a cradle that moves with the mothership while avoiding collision. This casts recovery as a two player differential game in which the control minimises a value and the bounded disturbance maximises it, and the value assigns to every state a number whose sublevel set is the set of interest. The control input is the rate of the thrust and the rudder, that is $(\dot\delta, \dot F)$, while the thrust and rudder amplitudes are carried in the state, so the game respects the actuator rate limits as well as the amplitude limits.

The reach avoid problem asks the system to reach the target set while never entering the failure set along the way, under the worst case disturbance \citep{margellos2011hamilton, fisac2015reach, hsu2021safety}. With the target margin $\ell$ and the failure margin $g$ of \eqref{eq_margins}, the reach avoid outcome of a trajectory is the payoff, and the value is its optimum over the control.
