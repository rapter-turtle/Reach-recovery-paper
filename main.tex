...
The problem is posed in a cradle fixed frame that advances with the mothership at $V_s$. The state $s = [x_r, y_r, \psi_r, u, v, r, \delta, F]^\top \in \mathbb{R}^8$ collects the relative pose, the velocity, and the actuator states, the control is the pair of actuator rates $a = [\dot\delta, \dot F]^\top$, and the disturbance $w \in \mathcal{W}$ is bounded as in Section~\ref{sec_model}. Two sets describe the outcome through Lipschitz margin functions $\ell$ and $g$,
\begin{equation}\label{eq_margins}
\ell(s)\le 0 \iff s\in\mathcal{T},\qquad g(s)>0 \iff s\in\mathcal{F},
\end{equation}
where $\mathcal{T}$ is the set of states in which the USV sits inside the cradle pocket with an aligned heading and a gentle relative velocity, that is a completed docking, and $\mathcal{F}$ is the set of states in which the USV strikes the mothership hull or the cradle structure. Both are marked in the zoom inset of Fig.~\ref{fig_scenario}, and the concrete geometry and numeric thresholds appear in Section~\ref{sec_results}.

The objective is to drive the USV into $\mathcal{T}$ while its trajectory never enters $\mathcal{F}$, for every admissible disturbance. This is a reach avoid requirement rather than plain avoidance, because entering the moving cradle is the mission itself, and a policy that merely keeps clear of $\mathcal{F}$ would never dock. As shown in Fig.~\ref{fig_scenario}, the recovery is carried out in two phases, handed over as detailed in Section~\ref{sec_framework}.
