...
The proposed framework was validated in numerical simulation, where the learned policy is compared against baseline controllers over a Monte Carlo campaign of physical sea scenarios. All experiments use the identified USV model of Section~\ref{sec_model}, whose parameters are collected in Table~\ref{tab_params}, and the docking geometry collected in Table~\ref{tab_set} and illustrated in Fig.~\ref{fig_setgeom}. In Table~\ref{tab_params} the symbol $m$ is the vessel mass, $m_{11}$, $m_{22}$, and $m_{33}$ are the surge, sway, and yaw inertias with their added terms, $l_r$ is the moment arm from the center of gravity to the thruster, $F_{\max}$ is the maximum thrust, $\delta$ is the rudder range, $\dot\delta$ and $\dot F$ are the rudder and thrust rate limits, and the remaining symbols are the hydrodynamic damping coefficients of Section~\ref{sec_model}. The reach and avoid conditions are evaluated at a head point a fixed distance forward of the center of gravity, with position $(x_h, y_h)$, because the bow crosses the cradle mouth first. The target margin takes the box form
...
g(s) = \max_{i}\, g_i(s).
\end{equation}
Here each $g_i$ is the signed margin of one of the three cradle walls or the mothership side boundary, positive when that obstacle is violated, so $g$ takes the value of the most violated one. The clearance reported in the results is the distance to the nearest obstacle, the negative of the avoid margin $g$, which stays positive while the vehicle is clear and reaches zero at contact.
...
\begin{figure*}[t]
  ...
  \caption{Head point trajectories for five representative sea scenarios under each controller. Successful dockings are drawn solid and end with a star, while collisions and timeouts are drawn dashed and end with a cross or a square. Each panel shows the cradle pocket, the mothership side boundary, and the dynamic positioning point marked with a diamond. The proposed panel shades the reach avoid set at the dynamic positioning point, the states from which the learned policy recovers the vehicle, whereas the baseline panels shade the funnel control barrier corridor enforced by DOB-MPC with RaCBF and the ALOS CBF. The proposed policy docks in every scenario and each baseline exhibits its own failure mode.}
...
The learned policy is evaluated against the Phase 1 DOB-MPC with RaCBF carried through the entire docking \citep{park2026recovery}, run at two barrier gains, a nominal gain (1) and a conservative gain (2), and against an ALOS CBF guidance baseline. All controllers face the same set of physical sea scenarios, each a wind, wave, and current field drawn from a Fossen sea model, with identical initial states and disturbance seeds, so the runs differ only in the control law. Recovery outcome, recovery time, and worst clearance to the obstacles are summarized in Fig.~\ref{fig_montecarlo}, and the per call computation time in Table~\ref{tab_compute}.

A representative recovery is shown in Fig.~\ref{fig_representative}. In the left panel the vessel threads from the dynamic positioning point into the cradle, with Phase 1 handing over to Phase 2 at the instant the reach avoid value crosses into the set. In the right panel the value decreases through zero at the handoff, the thrust and rudder remain inside their amplitude limits, and the clearance stays positive throughout, so the recovery is safe and admissible at once.
...
Aggregate behavior over the scenario set appears in Fig.~\ref{fig_montecarlo}. In panel (a) the proposed two phase policy docks every scenario, whereas the DOB-MPC with RaCBF loses a few to collisions at the nominal gain and to timeouts at the conservative gain, and the ALOS CBF is weakest with many collisions and timeouts. The dependence on the barrier gain is therefore non monotone, since a soft barrier admits contact while a conservative one stalls the approach, and only a delicately tuned setting escapes both. The proposed policy carries the reach avoid value in place of a barrier gain and is invariant to that tuning. Panel (b) shows that the policy also recovers fastest on average with the tightest spread over the docked scenarios, since it noses straight into the pocket rather than circling before entry. Panel (c) shows that it keeps the largest worst case clearance and stays well clear of the obstacles, while the baselines that collide graze zero clearance. The computation times in Table~\ref{tab_compute} complete the comparison, measured on a desktop with an Intel Core i7 12700KF CPU, an NVIDIA RTX 5080 GPU, and 32 GB of memory, where the learned policy runs on the GPU and the model predictive and guidance baselines run on the CPU. The Phase 2 reach avoid set control is the fastest and most consistent, a single network evaluation with no online optimization that sits comfortably inside the control period.

The four panels of Fig.~\ref{fig_trajectories} overlay the head point trajectories of five representative sea scenarios, the same five for every controller, namely the proposed two phase policy, the DOB-MPC with RaCBF at a nominal barrier gain (1) and at a conservative barrier gain (2), and the ALOS CBF guidance. The RaCBF gain sets the two poles through which the barrier drives the safe set margin back toward zero. The nominal gain (1) places the poles at 0.04 and 0.40, so the barrier reacts softly and late and permits deep travel toward the cradle walls before it intervenes. The conservative gain (2) places them at 0.10 and 0.70, so the barrier reacts sooner and holds a wider standoff from the walls. The proposed policy threads all five scenarios into the pocket along smooth head point paths that converge as they near the cradle. At the nominal gain the RaCBF baseline docks most runs, but one trajectory grazes the cradle wall near the pocket, where the barrier demands a lateral correction that the unactuated sway dynamics cannot supply. At the conservative gain the wall contacts disappear, but one run stalls short of the pocket and loops without entering, because the wider standoff holds the hull off the cradle. The ALOS guidance is the least reliable, with runs that veer toward the mothership side boundary and breach it and others that wander before entry. The barrier baselines therefore trade one failure mode for another as the gain varies, soft action admits contact while conservative action blocks entry, and the proposed policy avoids both.
...
$m$          & $1.5\times10^{3}$ [kg]         & $X_u$        & 134     \\
$m_{11}$     & $1.59\times10^{3}$ [kg]        & $X_{u|u|}$   & 31.0    \\
$m_{22}$     & $2.03\times10^{3}$ [kg]        & $Y_v$        & 98.2    \\
$m_{33}$     & $5.0\times10^{3}$ [kg\,m$^2$]  & $Y_{v|v|}$   & 200     \\
$l_r$        & 3.5 [m]                        & $Y_r$        & 306     \\
$F_{\max}$   & 0.86 [kN]                      & $N_v$        & 0       \\
$\delta$     & $\pm 57.3$ [deg]               & $N_r$        & $3.47\times10^{3}$ \\
$\dot\delta$ & $\pm 45.8$ [deg/s]             & $N_{r|r|}$   & 0       \\
$\dot F$     & $\pm 0.36$ [kN/s]              &              &         \\
\hline
\end{tabular}}
\end{table}
