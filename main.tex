...
\textit{Autonomous docking, berthing, and marine recovery.} Optimization based docking and berthing of surface vessels has been demonstrated with direct optimal control and model predictive control, including an optimization based collision free planner combined with dynamic positioning tracking validated in full scale experiments \citep{martinsen2019docking, bitar2020docking}, and end to end reinforcement learning has achieved autonomous docking of surface vehicles in unknown environments with validation on real platforms \citep{pereira2024illegal}. Reachability analysis has been applied to ship docking corridors against a static berth \citep{park2020reachability}. Underactuated marine craft guidance relies on adaptive line of sight path following with sideslip compensation of drift forces \citep{fossen2015los}, with standard vessel models following \citet{fossen2011handbook}. Launch and recovery to a platform has mostly been studied for underwater vehicles returning to a mother ship, including a docking and recovery system designed and proven in sea trials with a recovery mother ship \citep{zhang2023uuvrecovery}. Recovery of a surface vehicle onto a moving mothership has received far less attention. The authors' own prior controller handles the approach and station keeping with a disturbance observer based model predictive controller and a funnel type robust adaptive control barrier function \citep{park2026recovery}, but that barrier does not certify input admissibility during the terminal entry, and closing that gap is the contribution of this paper.

\textit{Robust predictive and barrier based safety.} Robust model predictive control keeps the state inside a tube around a nominal trajectory through an ancillary control law and a robust invariant set \citep{langson2004tube, mayne2005robust}, but such invariant sets are difficult to construct and conservative for nonlinear underactuated vessels. Disturbance observer based control, such as L1 adaptive control \citep{hovakimyan2010l1} and disturbance observer based model predictive control \citep{tomera2021dob}, improves tracking under environmental load but does not by itself certify safe reaching. Control barrier functions enforce safety as forward invariance and preserve it in an input to state safety sense under bounded perturbations \citep{xu2015robustness}, and related constructions include robust control barrier value functions \citep{choi2021robust} and HJ based refinement of hand designed barriers \citep{tonkens2022refining}. HJ values have also served as safety filters \citep{borquez2024safety}, and control barrier functions have been embedded as constraints inside model predictive control \citep{zeng2021safety, zeng2021enhancing, wang2025safety}. These approaches share a common limitation. They are predominantly avoid only, they enforce safety one step at a time or filter a nominal controller, and they do not by themselves guarantee reaching a moving target and can lose recursive feasibility under underactuation and actuator saturation. A further gap is that they constrain the input in amplitude only and act instantaneously, so actuator rate limits are not represented, whereas the present formulation augments the state with the actuator variables so that amplitude and rate limits are both respected within the reach avoid set.
