...
\begin{figure}[t]
  ...
  \caption{Learned robust reach avoid set in the cradle relative head point plane at three headings and the docking surge speed. The shaded region is where the value $V$ is negative, the states from which the trained policy drives the USV into the cradle under the worst case disturbance, and the zero level curve is the set boundary. The cradle and the fixed mothership side boundary are overlaid, and keeping the mothership wall in the avoid margin during training holds the set clear of the ship interior.}
  \label{fig_set}
\end{figure}
