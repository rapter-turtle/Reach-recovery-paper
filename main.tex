...
\begin{figure*}[t]
  ...
  \caption{Head point trajectories for six representative scenarios, the same six for every controller, in the cradle relative frame. Docks are solid with a star, collisions and timeouts are dashed with a cross and a square, and the cradle pocket and the mothership side boundary are drawn. The proposed policy docks all six, while each baseline shows its failure mode.}
